...
version = anvilCodegenVersions.getSubmoduleVersionProvider(
    propertiesFileKey = "anvil_ksp_codegen_activity_inject_version",
    envVariableName = "ANVIL_KSP_CODEGEN_ACTIVITY_INJECT_VERSION",
).get()

android {
    namespace = "ru.pixnews.anvil.ksp.codegen.activity"
}

dependencies {
    api(libs.anvil.ksp.annotations)
    api(libs.anvil.ksp.annotations.optional)
    api(libs.dagger)
    api(libs.inject)
}
